...
## 🔴 Short Term (Next Up)
*The engine and the player both exist. They can't affect each other yet — that is the next item, and it is the last one before Medium Term.*
...
- **Player cost is not benchmarked.** A collision test reads at most 32 cells and a step runs a handful of them, against 518,400 cells of grid. It is far below measurement noise, and `grid_bench` numbers are unchanged. Revisit only if there is ever more than one body.
