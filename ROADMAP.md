...
- [x] **Headless test harness.** The simulation has no SDL dependency and is covered by tests wired into CTest — conservation of matter, density stratification, static materials, border sealing. Now three suites, one per module: `tests/test_grid.cpp`, `tests/test_player.cpp`, `tests/test_tool.cpp`, sharing one harness in `tests/test_util.h`.
...
## 🔴 Short Term (Next Up)
*The engine, the player, and now digging all exist. What remains here are the two artifacts that digging makes visible — both were written as predictions before there was anything to dig, and both should now be judged against how the game actually plays rather than against the guess.*
...
- [ ] **Structural collapse for Wood and Wall.** Both are `MoveKind::Static` today — permanently immovable no matter what's under them. Digging now makes that visible: cut the support out from under a slab of Wall or Wood and it hangs in mid-air, which reads as a bug rather than as a rule. Sand already collapses correctly, so the inconsistency is on screen and side by side. Not a `MoveKind` flip: a solid chunk collapsing should behave like a falling mass or break into rubble, not disperse into loose sliding grains the way Powder does, so this needs its own design pass, not reuse of the powder behaviour.
