...
[Waves](#-waves--sub-plans-that-preempt-the-tracks) — findings are absorbed here
without being interleaved into the tracks, which is what the split was reaching
for.

**Live work comes first, finished work goes to the bottom.** Everything above
*Shipped* is open; everything in it is closed and kept for its reasoning rather
than its status. An earlier revision interleaved the two and read as a task list
while being sixty percent retrospective — a reader could not find the next item
without scrolling past four completed sections written in the imperative.
...
pass](#correctness-pass)** that followed a full read of the source. Then **V5**
...
is picked up.** [Wave
3](#wave-3--the-brush-destroyed-water-and-the-elevator-it-was-hiding) closed on
...
to work here](#correctness-pass-lessons).
...
- [ ] **E8 — Toppling.** *Observed:* E3 named this as the follow-on it existed
  ...
  [Waves](#-waves--sub-plans-that-preempt-the-tracks). It was for a while
  ...
  *Unlocks:* fire that reads as burning rather than as flickering, and it closes
  the last loose end the correctness pass left behind.
    ...
    - **The fuel half is done, and it was the whole of what was left in this
      ...
      play across [waves 2b and 2c](#-waves--sub-plans-that-preempt-the-tracks)
...
- [x] **V1 — Transparent `Empty` and a backdrop layer.** *(done — see
  [Shipped](#v1v2-visual-foundation))*
...
- [x] **World reset hotkey.** *(shipped 2026-08-14 as `T1.4`, on `Ctrl`+`R`
  rather than `R` — see [T1](#t1-the-debug-tooling-batch))* Wipes the grid back
...
- [x] **Pause and single-step (`P` to toggle, `.` to step once while paused).**
  *(shipped 2026-08-14 as `T1.1`; the brush-while-paused question is answered by
  ...
  [T1](#t1-the-debug-tooling-batch))* Freezes the fixed-step loop so a collapse
...
- [x] **A free camera, or some other way to look at the world.** *(shipped
  2026-08-14 as `T1.2`, on `F` — see [T1](#t1-the-debug-tooling-batch))* Not in
...
- [x] **Cell inspector.** *(shipped 2026-08-14 as `T1.3`, on `I`; it found the
  ...
  [T1](#t1-the-debug-tooling-batch))* Extend the in-window HUD with the material
...
*Four items lifted out of [Presentation & Tooling](#sandbox--debug-tooling) by
...
- **The manual checklist is load-bearing and was not run.** README's launch step
  ...
  window](#sandbox--debug-tooling)**, which V2's verify step already wanted for
  a different reason.
