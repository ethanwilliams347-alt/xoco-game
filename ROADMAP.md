...
- [x] **Eight materials** proving the above: Sand, Water, Wall, Wood, Oil (floats on water), Steam (rises), Fire, Rubble. Each cost one table row, no engine changes.
...
- [x] **Structural collapse.** Wall and Wood no longer hang in mid-air when the ground is dug out from under them: an unsupported structure breaks into a new Rubble material (a Powder, denser than Sand) and falls. Which materials can collapse and what they break into is a `debris` column in the `MATERIALS` table, so it stays one table rather than a second list — `Count` means "never collapses", which is the right answer for everything that already falls on its own. Support is a flood fill through connected structure looking for one cell that is grounded, where grounded means the world floor or something solid that is not part of the same structure; powders bear load, liquids and gases do not. **Two decisions worth knowing.** First, it is checked *on disturbance only*, never as a global truth — a sweep of the world every step would cost more than the simulation it is attached to, and a world as authored is assumed to be standing up on purpose, so a floating platform placed with the brush stays where it was put until something touches it. Second, structures over 4,096 cells are assumed supported rather than judged, because a missed collapse is invisible while a wrong one turns a level into rubble. Covered by 14 tests in a fourth suite, `tests/test_collapse.cpp`. This is the first feature in the project with a measurable performance cost — see the table above.
...
| Scenario | Before chunking | After chunking | With reactions | With collapse | % of a 60 Hz frame |
|---|---|---|---|---|---|
| **settled** — a world at rest | 1.64 ms | 0.0001 ms | 0.0001 ms | **0.0001 ms** | ~0% |
| **sparse** — static terrain, one small falling blob | 0.27 ms | 0.069 ms | 0.056 ms | **0.062 ms** | 0.4% |
| **churning** — sand sinking through water, then settling | 2.59 ms | 2.83 ms | 2.41 ms | **2.54 ms** | 15.2% |
| **cascading** — nothing ever settles, sustained worst case | — | 13.6 ms | 10.0 ms | **10.58 ms** | **63.5%** |
| **burning** — a Wood slab permanently on fire | — | — | 0.42 ms | **0.45 ms** | 2.7% |

**Read this honestly — collapse is the first feature that actually cost something.** Reactions were free in every scenario that had nothing to react (`try_react` exits on its first check), and digging was free because it runs a few dozen cells on a cooldown. Collapse is different: `swap_elements` now asks whether the cell above each end of the move is structure, and `swap_elements` is the hottest path in the engine. That is a real **+6% on `cascading`** (10.0 → 10.58 ms) and +5% on `churning`, measured consistently across runs rather than once.

An early-out that skipped ends still holding something solid was written, measured as run-to-run noise, and deleted again rather than kept on the theory that it ought to help. The cost is one extra memory touch a row above the cell being moved, which is a cache miss more often than it is arithmetic, and it is not going to be optimised away by being clever with the predicate.

This is affordable and is being paid deliberately. `cascading` is a synthetic worst case the game does not produce, real gameplay sits near `sparse`/`burning`, and the two cheap wins in Engineering Notes (cell size, RNG) are both still untouched and both aimed squarely at this loop. Spend them when a scenario the *game* produces goes over budget — not now.
