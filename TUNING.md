...
| `SKIN_CELLS` / `FULL_END` / `DEPTH_END` | [surface_plane.h](src/render/surface_plane.h) | 4 / 256 / 320 cells | **How far the plane's value reaches into the terrain below its own surface** (V25, 2026-08-23). This is the near-ground half of the plane: the terrain below the player is blended toward the plane's value at its own depth, so the receding surface does not stop at the character. Measured in cells below the **air directly above the cell**, never below the top of its column — that distinction is the item's one real defect and it cost 22.9% of the near band, because at the spawn some columns carry a trunk 375 cells above the ground. `SKIN_CELLS` is the only one of the three set against feel rather than coverage: it keeps the walkable line's own material colour, and at 10 it put a visible notch directly under the feet (band 17 read 67 between the feet's 79 and band 18's 73 — the shelf edge again at a tenth of the size). Four is still deeper than one dig swing. `FULL_END`/`DEPTH_END` are set against the census in `plane_probe`, which reads **1.1% untouched, mean weight 246 of 255, 93.6% at full strength**; at 96/160 it read 22.9% untouched and 176. Raising them further reaches deeper caverns, whose floors are surfaces of their own and would gain a bright band underground. |
| `PLANE_FADE_END_T` | [surface_plane.h](src/render/surface_plane.h) | 125 (= t 1.25) | **Where the near-ground blend stops, stated as position along the plane rather than as a depth** (V25, 2026-08-23). t = 0 is the horizon and t = 1 the plane's near edge; past that the surface has ended. **This exists because `DEPTH_END` was being asked a question it cannot answer** — underground the horizon is off the top of the window, every row is past the near edge, and every cell is deep, so a depth bound alone either paints solid rock with the brightest tile row or is set so tight it stops working above ground. It fades across the quarter-depth rather than cutting for the same reason the depth ramp fades: a hard end is a horizontal line drawn across the terrain at whatever row the near edge reached. Nothing at the spawn reaches it — the near edge lands at 1089 against a 1080-tall window — so this is a knob for the descent, and it is the one to look at first if the near band ever appears where the camera is underground. |
...
- **2026-08-23** — **V25: the near ground becomes the plane, and this is the
  first retune in the file that is not a value.** `SKIN_CELLS` / `FULL_END` /
  `DEPTH_END` and `PLANE_FADE_END_T` in
  [surface_plane.h](src/render/surface_plane.h), new. **Reported by playtest
  session 11** — *"it looks like the ground i am standing on is a separate shelf
  sitting in front of a parallaxed background"* — and then specified directly:
  *"i want the ground that the player is on to act like the water in those files
  visually… the player is sat into the page on the water plane."* **The four
  value retunes before this one could not have fixed it**, and the reason is
  worth carrying: the reference puts 28–39% of the plane in front of the
  character and we put 0%, because terrain occludes every row below the feet.
  That is composition, not shading, and `notes/reference_observations.txt` ENTRY
  9 said so on 2026-08-16. The composed band ladder went from **25 / 30 / 35 /
  40 / 46 / 53 / 59 / 65 / 71 at the feet, then collapsing to 49 / 30 / 24** to
  **34 / 43 / 52 / 59 / 64 / 70 / 75 / 82 at the feet / 82 / 88 / 88** —
  monotone to the bottom edge. Two of the four numbers moved once on
  measurement: `SKIN_CELLS` 10 → 4 closed a notch under the feet, and
  `FULL_END`/`DEPTH_END` 96/160 → 256/320 once `PLANE_FADE_END_T` took over the
  underground guard they had been carrying badly. **No `.rec` cost** — the pass
  is render-side and `input_log::fingerprint` hashes cell state, not pixels.
  Owed to the tester as checklist gate 8. Entry in
